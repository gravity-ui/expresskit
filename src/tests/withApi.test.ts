...
import { withApi, ExpressKit } from '../'; 
...
const rejectInvalidApiConfig = {
    ...
    responses: { // Must define expected responses, even if error is primary test
        200: { schema: z.object({ name: z.string(), value: z.number() }), description: 'Successful response (not expected in this test).' },
        400: { schema: ErrorSchema, description: 'Invalid request body.' },
    },
};
...
const TypedJsonTestSchema = z.object({ // Schema for response
    id: z.number(),
    name: z.string(),
...
const ComprehensiveValidationApiConfig = {
    ...
    request: {
        ...
        headers: z.object({
            'x-api-version': z.string().regex(/^v[0-9]+$/, { message: 'X-API-Version header must be in format v1, v2, etc.' }),
            'x-trace-id': z.string().uuid({ message: 'X-Trace-ID header must be a valid UUID' }),
        }),
    },
    responses: {
        200: { schema: ComprehensiveValidationSchema, description: 'Comprehensive validation successful.' },
        400: { schema: ErrorSchema, description: 'Invalid input for params, query, or headers.' },
    },
};

const comprehensiveValidationController = withApi(ComprehensiveValidationApiConfig)(async (req, res) => {
...
describe('withApi', () => {
    ...
    describe('Manual Validation', () => {
        ...
        it('should fail manual validation with invalid data and return 400', async () => {
            const response = await request(app)
                .post('/manual-validation-fail')
                .send({quantity: 0}); // Invalid: quantity must be >= 1
    ...
    describe('Error Response Structure', () => {
        it('should include a details property with issues for validation errors', async () => {
            const response = await request(app)
                .post('/error-structure-test')
                .send({value: 'abc'}); // Invalid: value should be a number
            ...
            expect(response.body.issues).toEqual(
                expect.arrayContaining([
                    expect.objectContaining({ path: ['body', 'value'] })
                ])
            );
        });
    });
